...
public class Reporter : MonoBehaviour
{
	...
	bool isGestureDone()
	{
		...
		if (gestureLength > gestureBase && gestureSum.magnitude < gestureBase / 2) {
			gestureDetector.Clear();
			gestureCount++;
			if (gestureCount >= numOfCircleToShow || gestureCount >= numOfGM)
				return true;
		}
	...
	void Update()
	{
		...
		if (!show && isGestureDone()) {
			if (gestureCount >= numOfCircleToShow)
				doShow();
			else if (gestureCount >= numOfGM)
			{
				LogManager.LogInfo("UIManager.Instance.ShowUI(UI_GM)");
				if (!UIManager.Instance.GetUIShowState("UI_GM"))
				{
					UIManager.Instance.ShowUI("UI_GM");
				}
			}
		}
